...
void CToolView::Ready_Setting(void)
{
	...
	if (Engine::Get_NaviMgr()->ReadyNaviMgr(m_pDevice))
		return;
...
void CToolView::Ready_Component(void)
{
	...
	//// Mesh =============
	// BackGround
	pComponent = Engine::CStaticMesh::Create(m_pDevice, L"../../Client/Bin/Resource/Mesh/StaticMesh/Map/BossMap2/", L"BossMap.X");
	if (pComponent == NULL)
		return;
...
void CToolView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_NaviTool.UpdateData(TRUE);

	((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_MapTool.SetMousePos(point);
	((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_NaviTool.SetMousePos(point);
	m_pCamera->Update(1, 0.1f);

	((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_MapTool.UpdateData(FALSE);
	((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_NaviTool.UpdateData(FALSE);
...
void CToolView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_iToolType == 0)
	{
		if (GetAsyncKeyState(VK_F1) & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_MapTool.TriggerType = 0;

		if (GetAsyncKeyState(VK_F2) & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_MapTool.TriggerType = 1;

		if (GetAsyncKeyState(VK_F3) & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_MapTool.TriggerType = 2;

		if (GetAsyncKeyState('1') & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_MapTool.NotifyType = 0;

		if (GetAsyncKeyState('2') & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_MapTool.NotifyType = 1;

		if (GetAsyncKeyState('3') & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_MapTool.NotifyType = 2;

		if (GetAsyncKeyState('4') & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_MapTool.NotifyOptionType = 0;

		if (GetAsyncKeyState('5') & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_MapTool.NotifyOptionType = 1;

		if (GetAsyncKeyState('6') & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_MapTool.NotifyOptionType = 2;
	}
	else if (((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_iToolType == 1)
	{
		if (GetAsyncKeyState(VK_F1) & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_NaviTool.TriggerType = 0;

		if (GetAsyncKeyState(VK_F2) & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_NaviTool.TriggerType = 1;

		if (GetAsyncKeyState(VK_F3) & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_NaviTool.TriggerType = 2;

		if (GetAsyncKeyState('1') & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_NaviTool.NotifyType = 0;

		if (GetAsyncKeyState('2') & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_NaviTool.NotifyType = 1;

		if (GetAsyncKeyState('3') & 0x8000)
			((CMainFrame*)AfxGetMainWnd())->m_pMainForm->m_NaviTool.NotifyType = 2;
	}
