...
{
	if (m_pCamBone == NULL)
		m_pCamBone = ((Engine::CDynamicMesh*)Engine::Get_Management()->GetLayer(L"Layer_Player")->GetObject(0)->GetComponent(L"Com_Mesh"))->GetPartsMatrix("ValveBiped_Anim_Attachment_CAM");
...
void CDefaultCamera::KeyCheck( const _float& fDelta )
{
	...
	if(dwMouseMove = Engine::GetDIMouseMove(Engine::CInputDevice::DIMS_Z))
	{
		m_fDistance -= dwMouseMove / 20.f;
	}
...
void CDefaultCamera::SetUpCamera( void )
{
	...
	MyMat matCompute = *m_pCamBone * *m_pTarget;
	memcpy(&vPos, &matCompute.m[3][0], sizeof(MyVec3));
	...
	D3DXMatrixRotationAxis(&matRotAxisY, &MyVec3(0.f, 1.f, 0.f), D3DXToRadian(m_fAngle.y));
