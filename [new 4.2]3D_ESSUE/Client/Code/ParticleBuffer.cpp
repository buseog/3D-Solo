...
HRESULT CParticleBuffer::Initialize(void)
{
	...
	m_pDevice->CreateVertexDeclaration(g_VertexElemHardware, &g_pVertexDeclHardware);
	...
	m_pDevice->CreateVertexBuffer(5 * sizeof(PARTICLE_INSTANCE), 0, 0, D3DPOOL_MANAGED, &m_pVBInstance, 0);
...
{	
	...
	m_pDevice->SetVertexDeclaration(g_pVertexDeclHardware);

	m_pDevice->SetStreamSource(0, m_pVB, 0, sizeof(PARTICLEVTX));
	m_pDevice->SetStreamSourceFreq(0, D3DSTREAMSOURCE_INDEXEDDATA | 5);
...
void CParticleBuffer::Free(void)
{
	...
	Engine::Safe_Release(g_pVertexDeclHardware);
