...
HRESULT Engine::CDynamicMesh::Initialize(const _tchar* FilePath, const _tchar* FileName, const MyMat* pInitMat)
{
	if (pInitMat == NULL)
		D3DXMatrixIdentity(&m_matParent);
	else
		m_matParent = *pInitMat;
...
void Engine::CDynamicMesh::SetUpShader( LPD3DXEFFECT pEffect, const _uint& iPass )
{
	if (pEffect == NULL)
		return;
	...
	for ( ; iter != iter_end; ++iter)
	{
		...
		for (_ulong i = 0; i < (*iter)->NumMaterials; ++i)
		{
			pEffect->SetTexture("g_BaseTexture", (*iter)->pTextures[i]);
...
void Engine::CDynamicMesh::SetAnimation(const _uint& iIndex, const _float& fSpeed, const _double& dFrame)
{
	m_pAniController->SetAnimation(iIndex, fSpeed, dFrame);
}
...
_bool Engine::CDynamicMesh::CheckAnimation( void )
{
	return m_pAniController->CheckAnimation();
}

_double Engine::CDynamicMesh::ProgressAnimation( void )
{
	return m_pAniController->ProgressAnimation();
}

_double Engine::CDynamicMesh::ProgressPerAnimation( void )
{
	return m_pAniController->ProgressPerAnimation();
}
...
CDynamicMesh* Engine::CDynamicMesh::Create(LPDIRECT3DDEVICE9 pDevice, const _tchar* FilePath, const _tchar* FileName, const MyMat* pInitMat)
{
	CDynamicMesh* pDynamic = new CDynamicMesh(pDevice);

	if (FAILED(pDynamic->Initialize(FilePath, FileName, pInitMat)))
	{
