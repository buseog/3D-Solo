...
void Engine::CAnimationControl::SetAnimation(const _uint& iIndex)
{
	...
	m_pAnimationControl->KeyTrackEnable(m_dwTrack, FALSE, m_dTimeAcc + 0.3);
	m_pAnimationControl->KeyTrackWeight(m_dwTrack, 0.1f, m_dTimeAcc, 0.3, D3DXTRANSITION_LINEAR);
	m_pAnimationControl->KeyTrackSpeed(m_dwTrack, 1.f, m_dTimeAcc, 0.3, D3DXTRANSITION_LINEAR);

	m_pAnimationControl->SetTrackEnable(m_dwNewTrack, TRUE);
	m_pAnimationControl->KeyTrackWeight(m_dwNewTrack, 0.9f, m_dTimeAcc, 0.3, D3DXTRANSITION_LINEAR);
	m_pAnimationControl->KeyTrackSpeed(m_dwNewTrack, 1.f, m_dTimeAcc, 0.3, D3DXTRANSITION_LINEAR);
