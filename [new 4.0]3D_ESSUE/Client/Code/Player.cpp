...
, m_fEffectCool(0.f)
{
	ZeroMemory(m_bSkill, sizeof(_bool) * 5);
}
...
void CPlayer::Active( const _float& fDelta )
{
	...
	if (m_iStateID == SharpenCombo)
	{
		...
		if (m_pMeshCom->ProgressPerAnimation() > 0.31 && m_bSkill[2] == false)
		{
			...
			for (int i = 0; i < 3; ++i)
			{
				pEffect = CDust::Create(m_pDevice, i, m_pTransformCom->GetWorldMatrix(), m_pMeshCom->GetPartsMatrix("ValveBiped_Bip01_R_Foot"));
				Engine::Get_Management()->GetLayer(L"Layer_Player")->AddGameObject(pEffect);
			}
			m_bSkill[2] = true;
		}
		...
		if (m_pMeshCom->ProgressPerAnimation() > 0.36 && m_bSkill[4] == false)
		{
			...
			for (int i = 10; i < 13; ++i)
			{
				pEffect = CDust::Create(m_pDevice, i, m_pTransformCom->GetWorldMatrix(), m_pMeshCom->GetPartsMatrix("ValveBiped_Bip01_L_Foot"));
				Engine::Get_Management()->GetLayer(L"Layer_Player")->AddGameObject(pEffect);
			}
			m_bSkill[4] = true;
		}
	...
	if (m_iStateID == DiffusionShift)
	{
		if (m_pMeshCom->ProgressAnimation() > 0.1 && m_pMeshCom->ProgressAnimation() < 1.0)
		{
			...
			m_pTransformCom->Straight((Engine::MyVec3*)&vDir, m_fSpeed * fDelta * 20.f);
		}
	...
	if (m_iStateID == DoomSayer)
	{
		...
		if (m_pMeshCom->ProgressPerAnimation() > 0.3 && m_bSkill[0] == false)
		{
...
{
	if (m_pKeyMgr->KeyDown(VK_CONTROL))
	{
		CParticleBuffer* pEffect = CParticleBuffer::Create(m_pDevice);
		Engine::Get_Management()->GetLayer(L"Layer_Player")->AddGameObject(pEffect);
	}
	...
	if (m_pKeyMgr->KeyDown('5') && m_iStateID <= Attack01)
	{
		...
		MyMat matAttach = *m_pMeshCom->GetPartsMatrix("ValveBiped_Bip01_Spine2") * *m_pTransformCom->GetWorldMatrix();

		for (int i = 0; i < 4; ++i)
		{
			CDarkNight* pEffect = CDarkNight::Create(m_pDevice, i, (MyVec3*)&matAttach.m[3][0]);
			Engine::Get_Management()->GetLayer(L"Layer_Player")->AddGameObject(pEffect);
		}
	}
