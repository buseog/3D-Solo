...
void Engine::CMaterial::SetUpShader( LPD3DXEFFECT pEffect )
{
	if (pEffect == NULL)
		return;

	pEffect->SetVector("g_vMaterialDiffuse", (MyVec4*)&m_MtrInfo.Diffuse);
	pEffect->SetVector("g_vMaterialAmbient", (MyVec4*)&m_MtrInfo.Ambient);
	pEffect->SetVector("g_vMaterialSpecular", (MyVec4*)&m_MtrInfo.Specular);
	pEffect->SetFloat("g_fPower", m_MtrInfo.Power);
}
