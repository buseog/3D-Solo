...
_uint CLoading::Loading_Stage( void )
{
	...
	// LongBladeTrail
	pComponent = Engine::CTexture::Create(m_pDevice, L"../Bin/Resource/Texture/LongBladeTrail/LongBlade_Trail%d.tga", Engine::CTexture::TEX_NORMAL);
	if (pComponent == NULL)
		return E_FAIL;
	Engine::Get_ComponentMgr()->AddComponent(SCENE_STAGE, L"Component_Texture_LongBladeTrail", pComponent);
	// Satellite
	pComponent = Engine::CTexture::Create(m_pDevice, L"../Bin/Resource/Texture/Effect/Satellite%d.tga", Engine::CTexture::TEX_NORMAL);
	if (pComponent == NULL)
		return E_FAIL;
