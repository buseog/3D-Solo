...
void Engine::CAnimationControl::SetAnimation(const _uint& iIndex, const _float& fSpeed, const _double& dFrame)
{
	...
	m_pAnimationControl->AdvanceTime(m_dTimeAcc, NULL);
...
{
	_float fSpeedDelta = fDelta * m_fSpeed;

	m_pAnimationControl->AdvanceTime(fSpeedDelta, NULL);

	m_dTimeAcc += fSpeedDelta;
}

_bool Engine::CAnimationControl::CheckAnimation( void )
{
...
_double Engine::CAnimationControl::ProgressAnimation( void )
{
	D3DXTRACK_DESC TrackDesc;
	ZeroMemory(&TrackDesc, sizeof(D3DXTRACK_DESC));
	m_pAnimationControl->GetTrackDesc(m_dwTrack, &TrackDesc);

	return TrackDesc.Position;
}

_double Engine::CAnimationControl::ProgressPerAnimation( void )
{
	D3DXTRACK_DESC TrackDesc;
	ZeroMemory(&TrackDesc, sizeof(D3DXTRACK_DESC));
	m_pAnimationControl->GetTrackDesc(m_dwTrack, &TrackDesc);

	return TrackDesc.Position / m_dPeriod;
}
