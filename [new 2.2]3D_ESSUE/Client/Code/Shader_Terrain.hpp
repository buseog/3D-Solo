...
PS_OUT PS_MAIN(PS_IN In)
{
	...
	vector	vTexColor = tex2D(BaseSampler, In.vTexUV * 20.f);

	Out.vColor = (vTexColor * (g_vLightDiffuse * g_vMaterialDiffuse) * (In.vShade * (g_vLightDiffuse * g_vMaterialDiffuse)) + (In.vSpecular) * (g_vLightDiffuse * g_vMaterialDiffuse));
