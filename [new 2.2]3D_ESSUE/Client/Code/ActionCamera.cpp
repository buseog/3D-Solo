...
HRESULT CActionCamera::Initialize(CPlayer* pPlayer, Engine::CDynamicMesh* pCamMesh, const MyVec3* pUp, _float fFovy, _float fAspect, _float fNear, _float fFar )
{
	m_pPlayer = pPlayer;
	m_pCamMesh = pCamMesh;
	m_pBoneEye = m_pCamMesh->GetPartsMatrix("ValveBiped_Anim_Attachment_CAM_Target");
	m_pBoneAt = m_pCamMesh->GetPartsMatrix("ValveBiped_Anim_Attachment_CAM");
...
_int CActionCamera::Update( const _float& fDelta )
{
	...
	MyMat matRot, matTrans, matWorld;
	//D3DXMatrixTranslation(&matTrans, 0.f, 0.f, -180.f);
//	D3DXMatrixRotationY(&matRot, D3DXToRadian(-120.f));
