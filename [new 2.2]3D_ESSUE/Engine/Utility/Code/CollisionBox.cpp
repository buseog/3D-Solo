...
void Engine::CCollisionBox::Render( COLLTYPE eType, _bool bColl, const MyMat* pMatWorld )
{
	if (eType == COLL_AABB)
	{
...
{
	m_tOBB.vCenter = (m_vMax + m_vMin) / 2.f;

	m_tOBB.vAxis[0] = m_vMax - MyVec3(m_vMin.x, m_vMax.y, m_vMax.z);
	m_tOBB.vAxis[1] = m_vMax - MyVec3(m_vMax.x, m_vMin.y, m_vMax.z);
	m_tOBB.vAxis[2] = m_vMax - MyVec3(m_vMax.x, m_vMax.y, m_vMin.z);

	for (_uint i = 0; i < 3; ++i)
	{
		D3DXVec3TransformCoord(&m_tOBB.vAxis[i], &m_tOBB.vAxis[i], &m_MatParent);
		D3DXVec3Normalize(&m_tOBB.vAxis[i], &m_tOBB.vAxis[i]);
	}

	ZeroMemory(m_tOBB.vPoint, sizeof(MyVec3) * 3);
	m_tOBB.vPoint[0].x = m_vMax.x - m_tOBB.vCenter.x;
	m_tOBB.vPoint[1].y = m_vMax.y - m_tOBB.vCenter.y;
	m_tOBB.vPoint[2].z = m_vMax.z - m_tOBB.vCenter.z;

	for (_uint i = 0; i < 3; ++i)
		D3DXVec3TransformCoord(&m_tOBB.vPoint[i], &m_tOBB.vPoint[i], &m_MatParent);
}
