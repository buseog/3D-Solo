...
void CPlayer::KeyCheck( const _float& fDelta )
{
	...
	if (m_pKeyMgr->KeyDown('Z') && m_iStateID <= Attack01)
	{
		m_iStateID = RunSprint;
		m_pMeshCom->SetAnimation(m_iStateID);

		MyMat matWorld = *m_pTransformCom->GetWorldMatrix();
		MyVec3 vRight = *D3DXVec3Normalize((MyVec3*)&matWorld.m[0][0], (MyVec3*)&matWorld.m[0][0]);
		MyVec3 vLook = *D3DXVec3Normalize((MyVec3*)&matWorld.m[2][0], (MyVec3*)&matWorld.m[2][0]);

		m_vTemp[0] = MyVec3(0.f, 0.f, 0.f);
		m_vTemp[1] = *(MyVec3*)&matWorld.m[3][0];
		m_vTemp[1].y = -30.f;
		m_vTemp[2] = *(MyVec3*)&matWorld.m[3][0] + (vRight * 20.f);
		m_vTemp[2].y = 20.f;
		m_vTemp[3] = *(MyVec3*)&matWorld.m[3][0] + (vLook * 30.f);
		m_vTemp[3].y = 5.f;
		m_vTemp[4] = *(MyVec3*)&matWorld.m[3][0];
		m_vTemp[4].y = -30.f;
		m_fPower = 0.f;
	}
	if (m_iStateID == RunSprint)
	{
		m_fPower += 0.02f;
		D3DXVec3CatmullRom(&m_vTemp[0], &m_vTemp[1], &m_vTemp[2], &m_vTemp[3], &m_vTemp[4], m_fPower);
		m_pTransformCom->SetInfomation(Engine::CTransform::INFO_POS, &m_vTemp[0]);
...
void CPlayer::Active( const _float& fDelta )
{
	if (m_iStateID >= Attack01 && m_iStateID <= Attack05)
	{
		...
		CEffect* pEffect = CEffect::Create(m_pDevice, L"Test");
		Engine::Get_Management()->GetLayer(L"Layer_Player")->AddGameObject(pEffect);
		((Engine::CTransform*)pEffect->GetComponent(L"Com_Transform"))->SetInfomation(Engine::CTransform::INFO_POS, &m_vTemp[0]);
...
void CPlayer::Command( const _float& fDelta )
{
	...
	if (m_pKeyMgr->KeyDown(VK_LBUTTON))
	{
		...
		m_vTemp[0] = MyVec3(0.f, 0.f, 0.f);
		m_vTemp[1] = *(MyVec3*)&matWorld.m[3][0] - (vLook * 60.f);
		m_vTemp[2] = *(MyVec3*)&matWorld.m[3][0] + (vLook * 5.f) + (vRight * 8.f);
		m_vTemp[3] = *(MyVec3*)&matWorld.m[3][0] + (vLook * 5.f) - (vRight * 8.f);
		m_vTemp[4] = *(MyVec3*)&matWorld.m[3][0] - (vLook * 60.f);
		m_fPower = 0.f;
