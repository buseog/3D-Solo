...
HRESULT CRuinWave::Initialize( const _uint& iType, MyMat* pTarget, MyMat* pBone)
{
	...
	if (m_iType == 11)
		m_pBoss = ((Engine::CTransform*)Engine::Get_Management()->GetLayer(L"Layer_Boss")->GetObject(0)->GetComponent(L"Com_Transform"))->GetWorldMatrix();
...
_int CRuinWave::Update( const _float& fDelta )
{
	...
	{	
		if ((*iter)->bAlive == true)
		{
			if (m_iType == 11 && m_bColl == false)
			{
				MyVec3 vDif = *(MyVec3*)&m_pBoss->m[3][0] - (*iter)->vPos;
				_float fDis = fabs(D3DXVec3Length(&vDif));
				_float fRadius = ((*iter)->vScale.x + (*iter)->vScale.y) * 2.f;

				if (fDis <= fRadius)
				{
					((CBoss*)Engine::Get_Management()->GetLayer(L"Layer_Boss")->GetObject(0))->SetDamage(150.f);
					m_bColl = true;
				}
			}
...
void CRuinWave::AddParticle( const _uint& iIndex )
{
	...
	case 20:
		...
		pEffect->vColor = MyVec3(255.f, 160.f, 90.f);
		pEffect->fMaxLife = 0.8f + _float(iIndex * 0.1f);
		...
		fSize = Engine::GetRandomFloat(3.f, 5.f);
		pEffect->vScale = MyVec3(fSize , fSize , 1.f);
		pEffect->vPos = vPos + vLook * (10.f + (_float)iIndex * 6.f);
	...
	case 21:
		pEffect->vAngle = MyVec3(Engine::GetRandomFloat(0.f, 10.f), Engine::GetRandomFloat(-360.f, 360.f), Engine::GetRandomFloat(-360.f, 360.f));
		pEffect->vColor = MyVec3(255.f, 160.f, 90.f);
		pEffect->fMaxLife = 0.9f + _float(iIndex * 0.1f);
	...
	case 22:
		...
		pEffect->vColor = MyVec3(255.f, 160.f, 90.f);
		pEffect->fMaxLife = 0.8f + _float(iIndex * 0.1f);
		pEffect->fSpeed = Engine::GetRandomFloat(5.f, 20.f);
		pEffect->fAngle = 0.f;
		pEffect->fRadius = Engine::GetRandomFloat(5.f, 15.f);
		pEffect->fRadiusAngle = 0.f;
		pEffect->vDir.z = pEffect->fRadius * sinf(pEffect->fAngle) * cosf(pEffect->fRadiusAngle);
	...
	case 23:
		...
		pEffect->vColor = MyVec3(255.f, 160.f, 90.f);
		pEffect->fMaxLife = 0.2f + _float(iIndex * 0.1f);
		pEffect->fSpeed = Engine::GetRandomFloat(0.f, 1.f);
		pEffect->fAngle = 0.f;
		pEffect->fRadius = Engine::GetRandomFloat(15.f, 25.f);
		pEffect->fRadiusAngle = 0.f;
		pEffect->vDir.z = pEffect->fRadius * sinf(pEffect->fAngle) * cosf(pEffect->fRadiusAngle);
		pEffect->vDir.x = pEffect->fRadius * sinf(pEffect->fAngle) * sinf(pEffect->fRadiusAngle);
		pEffect->vDir.y = pEffect->fRadius * cosf(pEffect->fAngle);
		fSize = Engine::GetRandomFloat(54.f, 67.f);
		pEffect->vScale = MyVec3(fSize , fSize , 1.f);
		pEffect->vPos = vPos + vLook * (10.f + (_float)iIndex * 6.f);
	...
	case 24:
		...
		pEffect->fAlphaDelta = 0.f;
		pEffect->fAlphaWayTime = 0.05f;
		pEffect->fAlphaWayDelta = -175.f;
		break;

	case 25:
		pEffect->vAngle = MyVec3(Engine::GetRandomFloat(80.f, 90.f), Engine::GetRandomFloat(-360.f, 360.f), Engine::GetRandomFloat(-360.f, 360.f));
		pEffect->vColor = MyVec3(255.f, 160.f, 90.f);
		pEffect->fMaxLife = 1.f + _float(iIndex * 0.1f);
		pEffect->fSpeed = Engine::GetRandomFloat(0.f, 1.f);
		pEffect->fAngle = 20.f;
	...
	D3DXVec3Normalize(&pEffect->vDir, &pEffect->vDir);
	if (m_iType < 20)
		pEffect->vPos += vLook * _float(_int(m_fDis / 0.3f) * 10);
	pEffect->vPos += MyVec3(0.f, 3.f, 0.f);
