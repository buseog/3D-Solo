...
HRESULT CSharpenCombo::Initialize( const _uint& iType, MyMat* pTarget, MyMat* pBone)
{
	m_iType = iType;
	m_pBone = pBone;
	m_pTarget = pTarget;
...
void CSharpenCombo::SetUpShader( LPD3DXEFFECT pEffect )
{
	pEffect->AddRef();
	...
	pEffect->SetVector("g_vCamPos", &MyVec4(*Engine::Get_CameraMgr()->GetCameraEye(), 1.f));
...
void CSharpenCombo::DefaultSetting( void )
{
	switch (m_iType)
	{
	case 0: // °ËÀº±×¸²ÀÚ
	...
	case 4: // °ËÀº±×¸²ÀÚ
		m_pEffectKey = L"Component_Texture_Smoke";
		m_iCount = rand() % 2 + 1;
		m_iIndex = 3;
...
void CSharpenCombo::AddParticle( void )
{
	...
	case 0:
		...
		pEffect->fAngle = 0.f;
		pEffect->fRadius = Engine::GetRandomFloat(20.f, 45.f);
		...
		fSize = Engine::GetRandomFloat(4.3f, 5.6f);
		pEffect->vScale = MyVec3(fSize, fSize, fSize);
		pEffect->vPos = vPos;

		pEffect->fSizeDelta = 5.f;
		pEffect->fAlphaWayDelta = 0.f;

		pEffect->bAlive = true;
		break;

	case 1:
		pEffect->vColor = MyVec3(55.f, 25.f, 25.f);
		pEffect->vAngle = MyVec3(10.f, 0.f, Engine::GetRandomFloat(-360.f, 360.f));
	...
	case 2:	
		...
		pEffect->fSpeed = Engine::GetRandomFloat(20.f, 25.f);
		pEffect->fAngle = 0.f;
		...
		pEffect->vDir.y = pEffect->fRadius * cosf(pEffect->fAngle);
		D3DXVec3Normalize(&pEffect->vDir, &pEffect->vDir);
		pEffect->vScale = MyVec3(Engine::GetRandomFloat(4.f, 5.f), Engine::GetRandomFloat(8.f, 9.f), 1.f);
		pEffect->vPos = vPos;
	...
	case 3:	
		...
		pEffect->vDir.y = pEffect->fRadius * cosf(pEffect->fAngle);
		D3DXVec3Normalize(&pEffect->vDir, &pEffect->vDir);
		pEffect->vScale = MyVec3(Engine::GetRandomFloat(7.f, 12.f), Engine::GetRandomFloat(11.f, 15.f), fSize);
		pEffect->vPos = vPos;
	...
	case 4:
		...
		pEffect->fSpeed = Engine::GetRandomFloat(25.f, 45.f);
		pEffect->fAngle = D3DXToRadian(360.f);
		...
		pEffect->vScale = MyVec3(fSize, fSize, fSize);
		pEffect->vAngle.y = Engine::GetRandomFloat(-360.f, 360.f);
		pEffect->vPos = m_vOrigin;
	...
	case 5:
		pEffect->fMaxLife = Engine::GetRandomFloat(0.4f, 0.6f);
		pEffect->fSpeed = Engine::GetRandomFloat(3.f, 10.f);
		pEffect->fAngle = 0.f;
		pEffect->fRadius = Engine::GetRandomFloat(20.f, 35.f);
		pEffect->fRadiusAngle = D3DXToRadian(360.f);
		pEffect->vDir.z = pEffect->fRadius * sinf(pEffect->fAngle) * cosf(pEffect->fRadiusAngle);
		pEffect->vDir.x = pEffect->fRadius * sinf(pEffect->fAngle) * sinf(pEffect->fRadiusAngle);
		pEffect->vDir.y = pEffect->fRadius * cosf(pEffect->fAngle);
		D3DXVec3Normalize(&pEffect->vDir, &pEffect->vDir);
		fSize = Engine::GetRandomFloat(6.f, 9.f);
		pEffect->vScale = MyVec3(fSize, fSize, fSize);
		pEffect->vAngle.y = Engine::GetRandomFloat(-360.f, 360.f);
		pEffect->vPos = vPos;
	...
	case 10:
		...
		pEffect->vDir.y = pEffect->fRadius * cosf(pEffect->fAngle);
		D3DXVec3Normalize(&pEffect->vDir, &pEffect->vDir);
		pEffect->vScale = MyVec3(25.f, 25.f, 25.f);
		pEffect->vPos = m_vOrigin;
	...
	case 12:
		pEffect->vAngle = MyVec3(Engine::GetRandomFloat(0.f, 30.f), 0.f, Engine::GetRandomFloat(-360.f, 360.f));
		pEffect->vColor = MyVec3(131.f, 149.f, 250.f);
		...
		pEffect->vDir.y = pEffect->fRadius * cosf(pEffect->fAngle);
		D3DXVec3Normalize(&pEffect->vDir, &pEffect->vDir);
		pEffect->vScale = MyVec3(2.f, 2.f, 2.f);
		pEffect->vPos = m_vOrigin;
	...
	case 22:
		pEffect->vAngle = MyVec3(0.f, -10.f, Engine::GetRandomFloat(-360.f, 360.f));
		pEffect->fMaxLife = 0.2f;
		...
		pEffect->fSizeDelta = Engine::GetRandomFloat(3.f, 5.f);
		pEffect->fSizeWayTime = 0.4f;
		pEffect->fSizeWayDelta = Engine::GetRandomFloat(4.f, 8.f);
	...
	case 23:
		pEffect->vAngle = MyVec3(0.f, -10.f, Engine::GetRandomFloat(-360.f, 360.f));
		pEffect->fMaxLife = 0.2f;
		pEffect->fSpeed = Engine::GetRandomFloat(70.f, 90.f);
		pEffect->fAngle = 30.f;
		pEffect->fRadius = 0.f;
		pEffect->fRadiusAngle = 360.f;
		pEffect->vDir.z = pEffect->fRadius * sinf(pEffect->fAngle) * cosf(pEffect->fRadiusAngle);
		...
		pEffect->vDir.y = pEffect->fRadius * cosf(pEffect->fAngle);
		D3DXVec3Normalize(&pEffect->vDir, &pEffect->vDir);
		pEffect->vScale = MyVec3(Engine::GetRandomFloat(1.f, 1.5f), Engine::GetRandomFloat(10.f, 20.f), 1.f);
		pEffect->vPos = vPos;

		pEffect->fSizeDelta = Engine::GetRandomFloat(3.f, 5.f);
		pEffect->fSizeWayTime = 0.4f;
		pEffect->fSizeWayDelta = Engine::GetRandomFloat(-3.f, -7.f);
	...
	case 24:
		...
		pEffect->vDir.y = pEffect->fRadius * cosf(pEffect->fAngle);
		D3DXVec3Normalize(&pEffect->vDir, &pEffect->vDir);
		pEffect->vScale = MyVec3(Engine::GetRandomFloat(1.f, 1.5f), Engine::GetRandomFloat(5.f, 7.f), 1.f);
		pEffect->vPos = vPos;
	...
	case 25:
		...
		pEffect->fSpeed = Engine::GetRandomFloat(1.f, 3.f);
		pEffect->fAngle = 0.f;
		pEffect->fRadius = Engine::GetRandomFloat(0.f, 13.f);
		pEffect->fRadiusAngle = 360.f;
		pEffect->vDir.z = pEffect->fRadius * sinf(pEffect->fAngle) * cosf(pEffect->fRadiusAngle);
		pEffect->vDir.x = pEffect->fRadius * sinf(pEffect->fAngle) * sinf(pEffect->fRadiusAngle);
		pEffect->vDir.y = pEffect->fRadius * cosf(pEffect->fAngle);
...
HRESULT CSharpenCombo::Ready_Component( void )
{
...
CSharpenCombo* CSharpenCombo::Create( LPDIRECT3DDEVICE9 pDevice, const _uint& iType, MyMat* pTarget, MyMat* pBone)
{
	CSharpenCombo* pSharpenCombo = new CSharpenCombo(pDevice);

	if (FAILED(pSharpenCombo->Initialize(iType, pTarget, pBone)))
	{
