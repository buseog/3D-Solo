...
HRESULT Engine::CStaticMesh::Initialize(const _tchar* FilePath, const _tchar* FileName)
{
	...
	if (dwSubSetCnt != 0)
	{
		for (_ulong i = 0; i < m_dwSubSetCnt; ++i)
		{
			...
			m_pSubSets[i].MatD3D.Diffuse.a = 1.f;
		}
	}
	else
	{
...
HRESULT Engine::CStaticMesh::Initialize( const _tchar* FilePath, const _tchar* FileName, const _uint* pIndex )
{
	_tchar szFullPath[128] = L"";
...
void Engine::CStaticMesh::SetUpShader( LPD3DXEFFECT pEffect, const _uint& iPass )
{
	if (pEffect == NULL)
		return;
...
CStaticMesh* Engine::CStaticMesh::Create( LPDIRECT3DDEVICE9 pDevice, const _tchar* FilePath, const _tchar* FileName, const _uint* pIndex )
{
	CStaticMesh* pStatic = new CStaticMesh(pDevice);

	if (FAILED(pStatic->Initialize(FilePath, FileName, pIndex)))
	{
		MSG_BOX("StaticMesh Created Failed");
		Engine::Safe_Release(pStatic);
	}
