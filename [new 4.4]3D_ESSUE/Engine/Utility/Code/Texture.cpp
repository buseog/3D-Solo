...
HRESULT Engine::CTexture::Initialize( const _tchar* pPath, TEXTURETYPE eType, const _uint& iCnt, const _uint* pIndex )
{
	m_vecTexture.reserve(iCnt);
...
void Engine::CTexture::SetUpShader( LPD3DXEFFECT pEffect, const _uint& iIndex )
{
	if (pEffect == NULL)
		return;
	...
	pEffect->SetTexture("g_BaseTexture", m_vecTexture[iIndex]);
}

void Engine::CTexture::SetUpShader( LPD3DXEFFECT pEffect, const char* pTexKey, const _uint& iIndex /*= 0*/ )
{
	if (pEffect == NULL)
		return;
	...
	pEffect->SetTexture(pTexKey, m_vecTexture[iIndex]);
}
...
CTexture* Engine::CTexture::Create( LPDIRECT3DDEVICE9 pDevice, const _tchar* pPath, TEXTURETYPE eType, const _uint* pIndex, const _uint& iCnt)
{
	CTexture* pTexture = new CTexture(pDevice);

	if (FAILED(pTexture->Initialize(pPath, eType, iCnt, pIndex)))
	{
		MSG_BOX("CTexture Created Failed");
		Engine::Safe_Release(pTexture);
	}
