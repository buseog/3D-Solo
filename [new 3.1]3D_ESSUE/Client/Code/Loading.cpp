...
_uint CLoading::Loading_Stage( void )
{
	...
	m_iCntLoad += 2;
	// Shader Train
	pComponent = Engine::CShader::Create(m_pDevice, L"../Code/Shader_Train.hpp");
	if (pComponent == NULL)
		return E_FAIL;
	Engine::Get_ComponentMgr()->AddComponent(SCENE_STAGE, L"Component_Shader_Train", pComponent);
	m_iCntLoad += 2;
